...
/*   Updated: 2020/04/17 00:53:02 by gmoon            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_horz(t_info *info, t_rc *rc, t_fd *horz)
{
	horz->is_horz = 1;
	horz->intersection_y =
	floor(info->y / info->tile_height) * info->tile_height;
	horz->intersection_y += (rc->is_ray_down) ? info->tile_height : 0;
	horz->intersection_x =
	info->x + (horz->intersection_y - info->y) / tan(rc->ray_angle);
...
void	init_vert(t_info *info, t_rc *rc, t_fd *vert)
{
	...
	vert->intersection_y =
	info->y + (vert->intersection_x - info->x) * tan(rc->ray_angle);
