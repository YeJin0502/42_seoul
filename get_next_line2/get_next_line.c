...
/*   Updated: 2020/04/06 07:31:37 by gmoon            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *gnl_strjoin(char *s1, char *s2) // 줄바꿈이나 널문자까지?
{
	...
	s2_size = (s2) ? strlen_until_lf(s2) : 0;
...
{
	gnl_memmove(buf,
				buf + strlen_until_lf(buf) + 1,
				gnl_strlen(buf) - strlen_until_lf(buf));
	// +1인가? 헷갈리네.
...
{
	int ret;
	static char *buf; // 여러줄 하려면 static으로 바꿔야할걸.
