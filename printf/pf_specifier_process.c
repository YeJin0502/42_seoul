...
/*   Updated: 2020/03/10 05:26:46 by gmoon            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	c_process(va_list ap, t_info info)
{
	...
	if (c_arg == 0)
		return (0);
	ret_size = (int)ft_strlen(c_arg);
	write(1, c_arg, ret_size);
	free(c_arg);
	return (ret_size);
}
...
int	p_process(va_list ap, t_info info)
{
	...
	if (arg == 0)
		...
		c_arg = ft_strjoin("0x", dec_to_hex((size_t)arg, info.spec)); // 덱투헥에서 누수 발생하지 않으려나?
	if (*(info.flag) != '\0')
